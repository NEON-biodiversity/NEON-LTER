...
osmu_burn<- readOGR(dsn = "./osbs_points/01osbs_points.shp")
...
# fire frequency
fire_dist<-apply(gDistance(osbs_UTM,osmu_burn,byid = TRUE),2,min)
fire_dist<-data.frame(fire_dist)
#rename column
names(fire_dist)[names(fire_dist)=="fire_dist"]<-"fire_dist"
head(fire_dist)
#merge data frame to make one complete dataset
osbs3<-merge(osbs_UTM,fire_dist,by=0,all=TRUE)
#check
head(fire_dist)
head(osbs3)
#Merge all data frames together for a complete data set
...
write.csv(combined.dist1,file="~/Documents/NEON_LTER_2018/osbs_dist.csv", row.names=FALSE)
##################################################################################################################################################################
#Now we need to clean up the data frame and select only the columns we need.
#Select important columns
keep=c("plotID", "siteID", "elevatn", "nlcdCls", "slpGrdn", "slpAspc","building_dist", "roads_dist","fire_dist")
...
keep[!keep %in% names(combined.dist1)]

combined.distances <- combined.dist1
...
combined.distances$dist_type[combined.distances$dist_type=="fire_dist"]<-"fire"
sort(unique(combined.distances$dist_type))
...
#fire area cacluation
fire_area<-gArea(osmu_burn)
fire_area
fire_area<-data.frame(fire_area)
fire_area
#rename column
names(fire_area)[names(fire_area)=="fire_area"]<-"fire_area"
head(fire_area)
#merge data
area_add2<-merge(combined.distances,fire_area,by=0,all=TRUE)
head(fire_area)
head(fire_add1)
#change all NA values to 0
...
head(area_add2)
#merge data frame
final1<-merge(final,area_add2,by=c("Row.names","plotID", "siteID", "elevatn", "nlcdCls", "slpAspc", "slpGrdn", "dist_type", "distance_m"), all=T)
names(final)

write.csv(final1,file="~/Documents/NEON_LTER_2018/osbs_final_plot_level.csv", row.names=FALSE)
##################################################################################################################################################################
#Now we need to clean up the data frame and select only the columns we need.
#Select important columns
keep=c("plotID", "siteID","elevatn", "nlcdCls", "slpAspc", "slpGrdn", "dist_type", "distance_m", "building_area","roads_area","fire_area")
...
keep[!keep %in% names(final1)]

plot_level_dataframe <- final1
